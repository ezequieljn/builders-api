...
describe("Test E2E Application", () => {
  ...
  function startApp(props: StartApp) {
    const paymentRepository =
      props.paymentRepository || new PaymentRepositoryMemory();
  ...
  it("should return error from validation of empty values", async () => {
    ...
    return request
      ...
      .expect(422)
      .expect({
        message: [{ bar_code: "Required" }, { payment_date: "Required" }],
      });
  });
  ...
  it("should return all saved payments", async () => {
    ...
    const paymentGateway = new PaymentSlipGatewayHttp(httpClientMock);
    const httpServer = startApp({
      authGateway: new AuthGatewayHttp(httpClientMock),
      paymentGateway: paymentGateway,
